...
> Only an owner can go back to personal mode, from Organization → Management → Return to personal mode.
> It permanently deletes every other user and all of their data, keeping only the owner who performs it.
...
| Role | Can do |
| --- | --- |
| **owner** | Everything an admin can, plus revert the whole instance to personal mode and make other users owners. The founder starts as owner; an organization can hold several owners later. |
...
- If an organization database ends up with no owner at all (every owner demoted, or a database predating the role), the oldest user is promoted to owner on the next startup.
- Owners are peers: only an owner can demote, suspend, or remove another owner, the last one included. Only an owner can revert to personal mode.
- There is always at least one active admin (owners count): the last one cannot be demoted, suspended, or removed.

## The Organization page

Admins get an Organization link in the sidebar, from which they can manage:

- **Users** — list everyone with a role dropdown (member / admin, plus owner for owners — choosing it makes that user an additional owner after a confirmation; the granting owner keeps their role).
...
- **Organization → Personal**: in app, *Organization → Management → Return to personal mode* (owner only action).
  This is destructive: it permanently deletes every account except the owner performing it (co-owners included) — cascade-deleting their vaults, categories, and sessions, dropping the organization tables (`org_vault`, `org_category`, `org_vault_membership`, `audit_event`, `invite`, `organization`) and reverting the mode.
